...
class Job
{
    ...
    public static function DailyJob()
    {
        ...
        foreach ($nodes as $node) {
            if ($node->sort == 0 || $node->sort == 10 || $node->sort == 11 || $node->sort == 12 || $node->sort == 13) {
                if (date("d") == $node->bandwidthlimit_resetday) {
                    ...
                    $node->save();
                    echo(date('Y-m-d H:i:s') . '节点[' . $node->name . ']重置流量完毕' . PHP_EOL);
                }
            }
        }
        ...
        echo(date('Y-m-d H:i:s') . '数据库被清理了，感觉身体被掏空了呢~' . PHP_EOL);
            ...
            if ($shop->reset() != 0 && $shop->reset_value() != 0 && $shop->reset_exp() != 0) {
                ...
                if (time() - $shop->reset_exp() * 86400 < $bought->datetime) {
                    if (intval((time() - $bought->datetime) / 86400) % $shop->reset() == 0 && intval((time() - $bought->datetime) / 86400) != 0) {
                        ...
                        $user->last_day_t = 0;
                        $user->save();
                        echo("流量重置-" . $user->id . PHP_EOL);

                        if (Config::get("mailDriver") != 'none') {
...
//                $bought->delete();
            ...
            if ($user->money >= $shop_price) {
                $user->money = $user->money - $shop_price;
                $user->save();
                $shop->buy($user, 1);
                $bought->renew = 0;
                $bought->save();
                ...
                foreach ($sub_users as $sub_user) {
                    ...
                    $bought_temp->shopid = $bought_new->shopid;
                    $bought_temp->datetime = $bought_new->datetime;
                    $bought_temp->renew = 0;
                    $bought_temp->price = 0;
                    $bought_temp->p_id = $bought_new->id;
                    $bought_temp->save();
